...
//Module to update the fd.channel.version in provisioning file
    Module fdChlProvisModule  = new Module.Builder('main/content')
        .withArtifact("packages",'main/content/target/*.zip',true)
...
//Smoke IT test cases need to be run for all the components, copy the section below in your component Jenkinsfile
	IntegrationTestRun smokeIT = new IntegrationTestRun.Builder()
		...
		IntegrationTestRun fdchannelItTests = new IntegrationTestRun.Builder()
		.withName('FD Channel IT Tests')
		.withBundle('main')
		.withNodeLabel(NODE_LABEL)
		.withInstance(author)
		.withTestCommand(new IntegrationTestRunCommand() {
			@Override
			void execute() {
				this.getJenkins().sh("mvn install")
...
config.setTestRuns([evergreenSmoke, smokeIT,fdchannelItTests]) //test cases to be run
