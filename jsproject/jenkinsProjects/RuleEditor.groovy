...
Module mainModule  = new Module.Builder('main')
    .withArtifact("bundles",'main/utils/target/*.jar', true)
	.withArtifact("packages",'main/rule-builder/target/*.zip',true)
    .build()
...
Module smokeITTests = new Module.Builder('smoke-it/basic')
    .withArtifact("it-tests-jar", 'smoke-it/basic/target/*.jar')
    .build()
	
//Module to update the expeditor.version in provisioning file
Module rulebuilderModule  = new Module.Builder('main/rule-builder')
    .withArtifact("packages",'main/rule-builder/target/*.zip',true)
...
List<Module> modules = [mainModule , smokeITTests, rulebuilderModule]
...
//Exp-Editor Dev unit test cases
...
config.setMailNotificationRecipients(['hug76386@adobe.com'])
